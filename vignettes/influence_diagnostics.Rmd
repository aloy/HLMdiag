...
```{r}
class.lme <- nlme::lme(mathgain ~ 1, random = ~ 1|schoolid/classid, data = class)
#hlm_influence(class.lme, level = 1)
```
...
Do I want an explanation of the full refit versus the approximation? (there's already a short explanation of it above)
...
cd_lme_full <- stats:::cooks.distance(lme.infl) #using lme4 throws an error, but using stats with the object created from lme4 works 
head(cd_lme_full)
head(cd_hlm_full)
```
`lme4`'s cook's distance function for objects created from it's influence function does not actually work. 
