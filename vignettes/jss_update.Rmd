---
title: "Updated code for JSS analysis"
authors: "Adam Loy"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{jss_update}
...
You can still access the `ggplot_qqnorm()` function, but it is no longer exported from the namespace.
```{r}
HLMdiag:::ggplot_qqnorm(x = resid_fm2$.ls.resid, line = "rlm")
```

Q-Q plots are now much easier to create in **ggplot2** directly
...
We can now use `hlm_influence` to pull off all of the case-deletion diagnostics at the specified level:
