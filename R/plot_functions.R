...
dotplot_diag <- function(
  ...
  if (!missing(name)) {
    if (
      ...
      stop(
        "name should be one of 'cooks.distance', 'mdffits', 'covratio', 
  ...
  if (!is.null(cutoff)) {
    if (!is.numeric(cutoff)) {
      cutoff <- internal_cutoff(x = x, name = name)
    }
    ...
    if (is.numeric(cutoff)) {
      ...
      if (modify == FALSE) {
        ...
        levels(df$n.factor)[
          levels(df$n.factor) %in% filter(df, .data$extreme == FALSE)$n.factor
        ] <- 0
        ...
        if (modify == "boxplot") {
          p <- p +
            geom_boxplot(
              aes(x = n.factor, y = value, group = 1),
              inherit.aes = FALSE
            )
        }
        if (modify == "dotplot") {
