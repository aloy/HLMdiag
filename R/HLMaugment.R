...
HLMaugment.lmerMod <- function(object, level = 1, standardize = FALSE, sim = NULL, ...) {
  ...
  if (level %in% names(object@flist)) {
    ...
    names(eb.resid)[1] <- "Int"
    groups <- rownames(eb.resid)
    if (standardize == TRUE) {
      se.re <- se.ranef(object)[[level]]
      eb.resid <- eb.resid/se.re
      eb.names <- paste(".std.ranef", names(eb.resid), sep = ".")
    } else {
      eb.names <- paste(".ranef", names(eb.resid), sep = ".")
    }
    
    # Assemble Tibble
    return.tbl <- tibble(groups, ls.resid, eb.resid, .name_repair = "universal")
    names(return.tbl) <- c(level, ls.names, eb.names)
    
    data <- object@frame
    if(class(data[,level]) != "character"){
      data[,level] <- as.character(data[,level])
    }
    fixed <- as.character( fixform( formula(object) ) )

    return.tbl <- tibble(inner_join(data, return.tbl))
