...
LSresids.default <- function(object, ...) {
  stop(paste(
    "there is no LSresids() method for objects of class",
...
LSresids.mer <- function(object, level, sim = NULL, standardize = FALSE, ...) {
  ...
  if (object@dims[["nest"]] == 0) {
    stop(
      "LSresids has not yet been implemented for models with 
  ...
  if (level != 1) {
    ...
    if (level == names(object@flist)[n.ranefs]) {
      # For highest level unit
      gform <- fixform(formula(object))
      global.model <- lm(formula = formula(gform), data = data)

      ls.models <- adjust_lmList(object = formula(form), data = data)
      ls.resid <- coef(ls.models)[, ranef_names] -
        coef(global.model)[ranef_names]
    } else {
      # For 'intermediate' level unit
      higher.level <- names(object@flist)[
        which(names(object@flist) == level) + 1
      ]
      ...
      global.coefs <- rep(
        unlist(coef(global.model)[ranef_names]),
...
LSresids.lmerMod <- function(object, level, standardize = FALSE, ...) {
  ...
  if (level == 1) {
    ...
    frame.lgl <-
      purrr::map(frame.split, function(split) {
        #check if column elements are equal
  ...
  if (level %in% names(object@flist)) {
    fixed <- as.character(reformulas::nobars(formula(object)))
    data <- object@frame
    ...
    if (level == names(object@flist)[n.ranefs]) {
      ...
      ls.models <- suppressWarnings(lme4::lmList(
        formula = formula(form),
        data = data
      ))
      if (sum(purrr::map_lgl(ls.models, is.null)) == length(ls.models)) {
        stop(
          "The model matrix is rank deficient. LS residuals cannot be calculated.",
          "\nUse the 'include.ls = FALSE' parameter to get EB residuals only."
        )
      }
      if (!is.null(attr(ls.models, which = "warningMsg"))) {
        warning(
          "The model matrix is likely rank deficient. Some LS residuals cannot be calculated.",
      ...
      if (ncol(ls.resid) != length(ranef_names) + 1) {
        warning(
          "The model contains a random effect term for variables with no fixed effect term.",
      ...
      split_data <- split_data[which(purrr::map_lgl(
        split_data,
        ~ (nrow(.x)) != 0
      ))]
      ...
      ls.models <- suppressWarnings(purrr::map(
        split_data,
        ~ lme4::lmList(formula = formula(form), data = .x)
      ))
      ls.warnings <- purrr::map_lgl(
        ls.models,
        ~ !is.null(attr(.x, which = "warningMsg"))
      )
      if (
        sum(purrr::map_lgl(ls.models, ~ all(purrr::map_lgl(.x, is.null)))) ==
          length(ls.models)
      ) {
        stop(
          "The model matrix is rank deficient. LS residuals cannot be calculated.",
