...
resid_marginal.default <- function(object, type) {
  stop(paste(
    "there is no resid_marginal() method for objects of class",
...
resid_conditional.default <- function(object, type) {
  stop(paste(
    "there is no resid_conditional() method for objects of class",
    paste(class(object), collapse = ", ")
  ))
}
...
#' @rdname resid_marginal
resid_marginal.lmerMod <- function(
  object,
...
#' @rdname resid_conditional
resid_conditional.lmerMod <- function(
  object,
  type = c("raw", "pearson", "studentized", "cholesky")
) {
  type <- match.arg(type)
...
#' @aliases resid_conditional
#' @rdname resid_conditional
resid_conditional.lme <- function(
  object,
  type = c("raw", "pearson", "studentized", "cholesky")
) {
  type <- match.arg(type)
...
resid_ranef.lmerMod <- function(object, level, which, standardize = FALSE) {
  ...
  if (standardize) {
    ...
    for (i in names(flist)) {
      diag_var <- diag(as.matrix(vc[[i]])) -
        diag(as.matrix(attr(re[[i]], "postVar")[,, 1]))
