...
hlm_influence.default <- function(model, ...) {
  stop(paste(
    "there is no hlm_influence() method for objects of class",
...
#' }
hlm_influence.lmerMod <- function(
  model,
  level = 1,
  delete = NULL,
  approx = TRUE,
  ...
  for (i in 1:length(leverage)) {
    if (!leverage[i] %in% c("overall", "fixef", "ranef", "ranef.uc")) {
      stop(paste(
        leverage[i],
        "is not a valid option for a type of leverage. Valid options are limited to: 'overall', 'fixef', 'ranef', or 'ranef.uc'."
      ))
    }
  }
  ...
  if (inherits(na.action, "exclude") & is.null(data)) {
    ...
    stop(
      "Please provide the data frame used to fit the model. This is necessary when the na.action is set to na.exclude."
    )
  }

  if (!isNestedModel(model) & approx == FALSE) {
    stop(
      "Full refits of the model are currently not implemented for models with crossed random effects."
    )
  }

  if (!is.null(delete)) {
    warning(
      "If the delete argument is specified, leverage cannot be returned. See ?hlm_influence for more information."
    )
  }

  if (approx) {
    #one step approximations
    infl.tbl <- tibble::tibble(
      cooksd = as.vector(cooks.distance(model, level = level, delete = delete)),
    ...
    if (isNestedModel(model)) {
      ...
      colnames(leverage.df) <- purrr::map_chr(leverage, function(s) {
        stringr::str_c("leverage", s, sep = ".")
      })
      infl.tbl <- tibble::add_column(infl.tbl, leverage.df)
    } else {
      warning(
        "Leverage is currently not implemented for models with crossed random effects."
      )
    }
    ...
    infl.tbl <- tibble::tibble(
      cooksd = as.vector(cooks.distance(case)),
    ...
    if (!is.null(delete)) {
      ...
      colnames(rvc.df) <- purrr::map_chr(names(rvc.df), function(s) {
        stringr::str_c("rvc", s, sep = ".")
      })
    ...
    if (level == 1) {
      ...
      infl.tbl <- tibble::add_column(
        infl.tbl,
        Group = unique(model@flist[[level]]),
        .before = 1
      )
      names(infl.tbl)[1] <- level
    }
...
hlm_influence.lme <- function(
  ...
  if (approx) {
    ...
    if (level == 1) {
      ...
      infl.tbl <- tibble::add_column(
        infl.tbl,
        Group = unique(model$groups[[level]]),
        .before = 1
      )
      names(infl.tbl)[1] <- level
    }
