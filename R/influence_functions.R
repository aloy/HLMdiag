...
leverage.default <- function(model, ...) {
  stop(paste(
    "there is no leverage() method for models of class",
...
covratio.default <- function(model, ...) {
  stop(paste(
    "there is no covratio() method for models of class",
    paste(class(model), collapse = ", ")
  ))
}
...
covtrace.default <- function(model, ...) {
  stop(paste(
    "there is no covtrace() method for models of class",
    paste(class(model), collapse = ", ")
  ))
}
...
mdffits.default <- function(model, ...) {
  stop(paste(
    "there is no mdffits() method for models of class",
    paste(class(model), collapse = ", ")
  ))
}
...
rvc.default <- function(model, ...) {
  stop(paste(
    "there is no rvc() method for models of class",
    paste(class(model), collapse = ", ")
  ))
}
...
leverage.mer <- function(model, level = 1, ...) {
  ...
  if (model@dims[["nest"]] == 0) {
    stop(
      "leverage.mer has not yet been implemented for models with 
  ...
  if (level == 1) {
    ...
    grp.lev <- data.frame(
      overall = grp.lev.fixef + grp.lev.ranef,
      fixef = grp.lev.fixef,
      ranef = grp.lev.ranef,
...
cooks.distance.mer <- function(model, level = 1, delete = NULL, ...) {
  ...
  if (hasArg(group)) {
    group <- NULL
    warning(
      "group is not a valid argument for this function. As of version 0.4.0, group has been replaced by level."
    )
  }
...
cooks.distance.lmerMod <- function(
  model,
  level = 1,
  delete = NULL,
  include.attr = FALSE,
...
mdffits.mer <- function(object, level = 1, delete = NULL, ...) {
  ...
  if (!is(object, "mer")) {
    stop("object must be of class 'mer'")
  }
  if (level != 1) {
