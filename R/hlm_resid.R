...
hlm_resid.default <- function(object, ...) {
  stop(paste(
    "there is no hlm_resid() method for objects of class",
...
hlm_resid.lmerMod <- function(
  ...
  if (!level %in% c(1, names(object@flist))) {
    stop(
      "level can only be 1 or the following grouping factors from the fitted model: \n",
  ...
  if (
    inherits(attr(object@frame, "na.action"), "exclude") &&
      is.null(data) &&
      level == 1
  ) {
  ...
  if (level == 1) {
    ...
    if (inherits(attr(object@frame, "na.action"), "exclude")) {
      if (include.ls == TRUE) {
        ...
        problem_dfs <- data.frame(
          MR = problem_dfs[order(as.numeric(rownames(problem_dfs))), ]
        )
  ...
  if (level %in% names(object@flist)) {
    ...
    if (level == names(object@flist)[n.ranefs]) {
      ...
      if (include.ls == TRUE) {
        ...
        if (
          sum(class(g.vars[level][[1]]) != class(return.tbl[level][[1]])) != 0
        ) {
...
hlm_resid.lme <- function(
  ...
  if (level == 1) {
    ...
    if (include.ls == TRUE) {
      return.tbl <- tibble::tibble(
        ".id" = as.numeric(rownames(model.data)),
        model.data,
        eb.resid,
        eb.fitted,
        ls.resid,
