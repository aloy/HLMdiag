...
#'data frame of influence measures depending on whether \code{type = "both"} or
#'if only one aspect of the model is selected. If \code{type = "both"}, then a
#'list with Cook's distance, MDFFITS, COVTRACE, and COVRATIO are returned for
#'the fixed effects and relative variance change (RVC) is returned for the
#'variance components.
#'
#'The functions \code{cooksd_hlm}, \code{mdffits_hlm}, \code{covtrace_hlm},
#'\code{covratio_hlm}, and \code{rvc} can be used for direct computation of the
