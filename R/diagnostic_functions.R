...
#' 
#' The primary function is \code{diagnostics} which returns either a list or
#' data frame of influence measures depending on whether \code{type = "both"}
#' or if only one aspect of the model is selected. If \code{type = "both"},
#' then a list with Cook's distance, MDFFITS, COVTRACE, and COVRATIO are
#' returned for the fixed effects and relative variance change (RVC) is
#' returned for the variance components.
#' 
#' The functions \code{cooksd_hlm}, \code{mdffits_hlm}, \code{covtrace_hlm},
#' \code{covratio_hlm}, and \code{rvc} can be used for direct computation of
#' the corresponding diagnostic quantities.
#' 
#' @aliases cooksd_hlm covratio_hlm covtrace_hlm diagnostics mdffits_hlm rvc
#' @param model an object contatining the original hierarchical model fit using
#' \code{lmer()}
#' @param delete an object containing the output returned by
#' \code{case_delete()}
