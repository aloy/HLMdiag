...
case_delete.default <- function(model, ...) {
  stop(paste(
    "there is no case_delete() method for objects of class",
...
case_delete.mer <- function(
  ...
  if (!is(model, "mer")) {
    stop("model must be of class 'mer'")
  }
  if (!model@dims["LMM"]) {
    stop(
      "case_delete is currently only implemented for mixed/hierarchical models."
    )
  }
  ...
  if (hasArg(group)) {
    group <- NULL
    warning(
      "group is not a valid argument for this function. As of version 0.4.0, group has been replaced by level. See ?hlm_influence for more information."
    )
  }
    ...
    if (is.null(delete)) {
      ...
      if (length(flist) == 1) {
        ranef.delete <- lapply(model.delete, function(x) {
          data.frame(
            deleted = setdiff(model@frame[, level], x@frame[, level]),
            id = rownames(lme4::ranef(x)[[1]]),
            lme4::ranef(x)[[1]]
          )
        })
      } else {
  ...
  if (length(ranef.original) == 1) {
    ranef.original <- ranef.original[[1]]
  }
  ...
  val <- list(
    fixef.original = fixef.original,
    ranef.original = ranef.original,
    vcov.original = vcov.original,
    varcomp.original = varcomp.original,
    fixef.delete = fixef.delete,
    ranef.delete = ranef.delete,
    vcov.delete = vcov.delete,
  ...
  if (level == 1) {
    ...
    if (is.null(delete)) {
      ...
      if (!is.numeric(delete)) {
        stop(
          "For individual case deletion, the delete parameter should be a numeric vector"
        )
      }
      model.delete <- lme4::lmer(
        formula = formula(model),
    ...
    if (is.null(delete)) {
      ...
      fitted.delete <- lapply(model.delete, function(x) {
        data.frame(
          deleted = setdiff(model@flist[[level]], x@flist[[level]]),
          x@frame,
          fitted(x)
        )
      })
      ...
      if (is.numeric(delete)) {
        ...
        for (i in seq_along(delete)) {
          if (!delete[i] %in% flist[[level]]) {
            stop(paste(
              delete[i],
              "is not a valid group name to delete for this model. Names should follow the same format as in model@flist[[group]]. An example of an acceptable group name is:",
              model@flist[[level]][1]
            ))
          }
        }
      ...
      if (sum(deleted_levels %in% model.delete@flist[[level]]) > 0) {
        warning(
          "Level parameter is specified, but deleted cases do not encompass entire groups"
        )
      }
  ...
  if (level == 1) {
    ...
    if (is.null(delete)) {
      ...
      if (length(flist) == 1) {
        ranef.delete <- lapply(model.delete, function(x) {
          data.frame(
            deleted = setdiff(modframe[, level], x$data[, level]),
            id = rownames(nlme::ranef(x)),
            nlme::ranef(x)
          )
        })
      } else {
      ...
      if (is.numeric(delete)) {
        ...
        for (i in seq_along(delete)) {
          if (!delete[i] %in% model$groups[[level]]) {
            stop(paste(
              delete[i],
              "is not a valid group name to delete for this model. 
                       Names should follow the same format as in model$groups[[level]]. 
                       An example of an acceptable group name is:",
              model$groups[[level]][[1]]
            ))
          }
        }
        index <- unlist(lapply(list(delete), function(s) {
