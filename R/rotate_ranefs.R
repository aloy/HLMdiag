...
rotate_ranef.default <- function(.mod, ...) {
  stop(paste(
    "there is no rotate_ranef() method for objects of class",
...
mcrotate <- function(A, B, s) {
  ...
  A.star <- Diagonal(x = 1 / sqrt(Cr.diag)) %*%
    t(Tr) %*%
    A %*%
    Tr %*%
    Diagonal(x = 1 / sqrt(Cr.diag))
...
.extract.lmeDesign <- function(m) {
  ...
  y <- as.vector(
    matrix(m$residuals, ncol = NCOL(m$residuals))[,
      NCOL(m$residuals)
    ] +
      matrix(m$fitted, ncol = NCOL(m$fitted))[,
        NCOL(m$fitted)
      ]
  )
  return(list(
