...
chem.lmer.caseG2 <- suppressMessages(case_delete(
  chem.lmer,
  level = "school:lea"
))
...
chem.lmer.caseGD2 <- case_delete(
  chem.lmer,
  level = "school:lea",
  delete = c("1:1", "5:2")
)
chem.lme.caseGD2 <- case_delete(
  chem.lme,
  level = "school",
  delete = c("1/1", "2/5")
)
...
test_that("Original predicted random effects match output from ranef for lme4 models", {
  ...
  expect_equal(
    sleep.lmer.caseG$ranef.original[[1]],
    ranef(sleep.lmer)[[1]][, 1]
  )
  expect_equal(
    sleep.lmer.caseG$ranef.original[[2]],
    ranef(sleep.lmer)[[1]][, 2]
  )

  #chemistry
  expect_equal(
    chem.lmer.case$ranef.original[[1]][, 1],
    ranef(chem.lmer)[[1]][, 1]
  )
  expect_equal(
    chem.lmer.case$ranef.original[[2]][, 1],
    ranef(chem.lmer)[[2]][, 1]
  )

  expect_equal(
    chem.lmer.caseG$ranef.original[[1]][, 1],
    ranef(chem.lmer)[[1]][, 1]
  )
  expect_equal(
    chem.lmer.caseG$ranef.original[[2]][, 1],
    ranef(chem.lmer)[[2]][, 1]
  )

  expect_equal(
    chem.lmer.caseG2$ranef.original[[1]][, 1],
    ranef(chem.lmer)[[1]][, 1]
  )
  expect_equal(
    chem.lmer.caseG2$ranef.original[[2]][, 1],
    ranef(chem.lmer)[[2]][, 1]
  )
})
...
test_that("Original predicted random effects match output from ranef for nlme models", {
  ...
  #sleepstudy
  expect_equal(
    chem.lme.case$ranef.original[[1]][, 1],
    ranef(chem.lme)[[1]][, 1]
  )
  expect_equal(
    chem.lme.case$ranef.original[[2]][, 1],
    ranef(chem.lme)[[2]][, 1]
  )

  expect_equal(
    chem.lme.caseG$ranef.original[[1]][, 1],
    ranef(chem.lme)[[1]][, 1]
  )
  expect_equal(
    chem.lme.caseG$ranef.original[[2]][, 1],
    ranef(chem.lme)[[2]][, 1]
  )

  expect_equal(
    chem.lme.caseG2$ranef.original[[1]][, 1],
    ranef(chem.lme)[[1]][, 1]
  )
  expect_equal(
    chem.lme.caseG2$ranef.original[[2]][, 1],
    ranef(chem.lme)[[2]][, 1]
  )
})
...
test_that("Dimensions of fixed effects after deletion are correct for single case deletion for lme4 models", {
  ...
  expect_equal(
    ncol(sleep.lmer.case$fixef.delete),
    1 + length(fixef(sleep.lmer))
  )
...
test_that("Dimensions of fixed effects after deletion are correct for group deletion for lme4 models", {
  ...
  #sleepstudy
  expect_equal(
    nrow(sleep.lmer.caseG$fixef.delete),
    length(unique(sleep.lmer@flist[["Subject"]]))
  )
  expect_equal(ncol(sleep.lmer.caseG$fixef.delete), length(fixef(sleep.lmer)))
  expect_equal(
    length(sleep.lmer.caseGD$fixef.delete),
    length(fixef(sleep.lmer))
  )

  #chemistry
  expect_equal(
    nrow(chem.lmer.caseG$fixef.delete),
    length(unique(chem.lmer@flist[["lea"]]))
  )
  expect_equal(ncol(chem.lmer.caseG$fixef.delete), length(fixef(chem.lmer)))
  expect_equal(
    nrow(chem.lmer.caseG2$fixef.delete),
    length(unique(chem.lmer@flist[["school:lea"]]))
  )
...
test_that("Dimensions of fixed effects after deletion are correct for group deletion for nlme models", {
  #sleepstudy
  expect_equal(
    nrow(sleep.lme.caseG$fixef.delete),
    length(unique(sleep.lme$groups$Subject))
  )
  ...
  #chemistry
  expect_equal(
    nrow(chem.lme.caseG$fixef.delete),
    length(unique(chem.lme$groups[["lea"]]))
  )
  expect_equal(ncol(chem.lme.caseG$fixef.delete), length(fixef(chem.lme)))
  expect_equal(
    nrow(chem.lme.caseG2$fixef.delete),
    length(unique(chem.lme$groups[["school"]]))
  )
...
test_that("Dimensions of random effects after deletion are correct for single case deletion for lme4 models", {
  ...
  #sleepstudy
  expect_equal(
    nrow(sleep.lmer.case$ranef.delete),
    nrow(sleep.lmer@frame) * length(unique(sleep.lmer@flist[["Subject"]]))
  )
  expect_equal(
    ncol(sleep.lmer.case$ranef.delete),
    2 + ncol(ranef(sleep.lmer)$Subject)
  )
  expect_equal(
    nrow(sleep.lmer.caseD$ranef.delete),
    length(unique(sleep.lmer@flist[["Subject"]]))
  )
  expect_equal(
    ncol(sleep.lmer.caseD$ranef.delete),
    ncol(ranef(sleep.lmer)$Subject)
  )

  #chemistry

  expect_equal(
    nrow(chem.lmer.case$ranef.delete[[1]]),
    -3 +
      (nrow(chem.lmer@frame) * length(unique(chem.lmer@flist[["school:lea"]])))
  )
  expect_equal(
    ncol(chem.lmer.case$ranef.delete[[1]]),
    2 + ncol(ranef(chem.lmer)$'school:lea')
  )

  expect_equal(
    nrow(chem.lmer.case$ranef.delete[[2]]),
    nrow(chem.lmer@frame) * length(unique(chem.lmer@flist[["lea"]]))
  )
  expect_equal(
    ncol(chem.lmer.case$ranef.delete[[2]]),
    2 + ncol(ranef(chem.lmer)$lea)
  )
})
...
test_that("Dimensions of random effects after deletion are correct for single case deletion for nlme models", {
  ...
  #sleepstudy
  expect_equal(
    nrow(sleep.lme.case$ranef.delete),
    nrow(sleep.lme$groups) * length(unique(sleep.lme$groups$Subject))
  )
  expect_equal(ncol(sleep.lme.case$ranef.delete), 2 + ncol(ranef(sleep.lme)))
  expect_equal(
    nrow(sleep.lme.caseD$ranef.delete),
    length(unique(sleep.lme$groups$Subject))
  )
  ...
  expect_equal(
    nrow(chem.lme.case$ranef.delete[[2]]),
    -3 + nrow(chem.lme$groups) * length(unique(chem.lme$groups[["school"]]))
  )
  expect_equal(
    ncol(chem.lme.case$ranef.delete[[2]]),
    2 + ncol(ranef(chem.lme)$'school')
  )

  expect_equal(
    nrow(chem.lme.case$ranef.delete[[1]]),
    nrow(chem.lme$groups) * length(unique(chem.lme$groups$lea))
  )
  expect_equal(
    ncol(chem.lme.case$ranef.delete[[1]]),
    2 + ncol(ranef(chem.lme)$lea)
  )
})
...
test_that("Dimensions of random effects after deletion are correct for group deletion when delete param is NULL for lme4 models", {
  ...
  expect_equal(
    ncol(sleep.lmer.caseG$ranef.delete),
    2 + ncol(ranef(sleep.lmer)$Subject)
  )
  ...
  expect_equal(
    ncol(chem.lmer.caseG$ranef.delete[[2]]),
    2 + ncol(ranef(chem.lmer)$lea)
  )
  ...
  expect_equal(
    ncol(chem.lmer.caseG2$ranef.delete[[1]]),
    2 + ncol(ranef(chem.lmer)$'school:lea')
  )
})
...
test_that("Dimensions of random effects after deletion are correct for group deletion when delete param is NULL for nlme models", {
  ...
  expect_equal(
    ncol(chem.lme.caseG$ranef.delete[[1]]),
    1 + ncol(ranef(chem.lme)$lea)
  ) #lme doesn't add delete column

  n <- length(unique(chem.lme$groups[["school"]]))
  expect_equal(nrow(chem.lme.caseG2$ranef.delete[[2]]), n * (n - 1))
  expect_equal(
    ncol(chem.lme.caseG2$ranef.delete[[2]]),
    1 + ncol(ranef(chem.lme)$school)
  )
})
...
test_that("Dimensions of random effects after deletion are correct for group deletion when delete param is set for lme4 models", {
  #sleepstudy
  expect_equal(
    nrow(sleep.lmer.caseGD$ranef.delete),
    length(unique(sleep.lmer@flist[["Subject"]])) - 1
  )
  expect_equal(
    ncol(sleep.lmer.caseGD$ranef.delete),
    ncol(ranef(sleep.lmer)$Subject)
  )

  #chemistry
  expect_equal(
    nrow(chem.lmer.caseGD$ranef.delete[[2]]),
    length(unique(chem.lmer@flist[["lea"]])) - 2
  )
  expect_equal(
    ncol(chem.lmer.caseGD$ranef.delete[[2]]),
    ncol(ranef(chem.lmer)$lea)
  )

  expect_equal(nrow(chem.lmer.caseGD$ranef.delete[[1]]), 12) #there are 12 schools left after removing 2 leas
  expect_equal(
    ncol(chem.lmer.caseGD$ranef.delete[[1]]),
    ncol(ranef(chem.lmer)$school)
  )

  expect_equal(
    nrow(chem.lmer.caseGD2$ranef.delete[[2]]),
    length(unique(chem.lmer@flist[["lea"]]))
  )
  expect_equal(
    ncol(chem.lmer.caseGD2$ranef.delete[[2]]),
    ncol(ranef(chem.lmer)$'lea')
  )

  expect_equal(
    nrow(chem.lmer.caseGD2$ranef.delete[[1]]),
    length(unique(chem.lmer@flist[["school:lea"]])) - 2
  )
  expect_equal(
    ncol(chem.lmer.caseGD2$ranef.delete[[1]]),
    ncol(ranef(chem.lmer)$'school:lea')
  )
})
...
test_that("Dimensions of random effects after deletion are correct for group deletion when delete param is set for nlme models", {
  #sleepstudy
  expect_equal(
    nrow(sleep.lme.caseGD$ranef.delete),
    length(unique(sleep.lme$groups$Subject)) - 1
  )
  expect_equal(ncol(sleep.lme.caseGD$ranef.delete), ncol(ranef(sleep.lme)))

  #chemistry
  expect_equal(
    nrow(chem.lme.caseGD$ranef.delete[[1]]),
    length(unique(chem.lme$groups[["lea"]])) - 2
  )
  expect_equal(
    ncol(chem.lme.caseGD$ranef.delete[[1]]),
    ncol(ranef(chem.lme)$lea)
  )

  expect_equal(nrow(chem.lme.caseGD$ranef.delete[[2]]), 12) #there are 12 schools left after removing 2 leas
  expect_equal(
    ncol(chem.lme.caseGD$ranef.delete[[2]]),
    ncol(ranef(chem.lme)$school)
  )

  expect_equal(
    nrow(chem.lme.caseGD2$ranef.delete[[1]]),
    length(unique(chem.lme$groups[["lea"]]))
  )
  expect_equal(
    ncol(chem.lme.caseGD2$ranef.delete[[1]]),
    ncol(ranef(chem.lme)$'school')
  )

  expect_equal(
    nrow(chem.lme.caseGD2$ranef.delete[[2]]),
    length(unique(chem.lme$groups[["school"]])) - 2
  )
  expect_equal(
    ncol(chem.lme.caseGD2$ranef.delete[[2]]),
    ncol(ranef(chem.lme)$lea)
  )
})
...
test_that("Dimensions of variance covariance matrices after deletion are correct for single case deletion for lme4 models", {
  ...
  expect_equal(
    nrow(sleep.lmer.case$vcov.delete[[1]]),
    nrow(as.matrix(vcov(sleep.lmer)))
  )
  expect_equal(
    ncol(sleep.lmer.case$vcov.delete[[1]]),
    ncol(as.matrix(vcov(sleep.lmer)))
  )
  expect_equal(
    nrow(sleep.lmer.caseD$vcov.delete),
    nrow(as.matrix(vcov(sleep.lmer)))
  )
  expect_equal(
    ncol(sleep.lmer.caseD$vcov.delete),
    ncol(as.matrix(vcov(sleep.lmer)))
  )
  ...
  expect_equal(
    nrow(chem.lmer.case$vcov.delete[[1]]),
    nrow(as.matrix(vcov(chem.lmer)))
  )
  expect_equal(
    ncol(chem.lmer.case$vcov.delete[[1]]),
    ncol(as.matrix(vcov(chem.lmer)))
  )
})
...
test_that("Dimensions of variance covariance matrices after deletion are correct for single case deletion for nlme models", {
  ...
  expect_equal(
    nrow(sleep.lme.case$vcov.delete[[1]]),
    nrow(as.matrix(vcov(sleep.lme)))
  )
  expect_equal(
    ncol(sleep.lme.case$vcov.delete[[1]]),
    ncol(as.matrix(vcov(sleep.lme)))
  )
  expect_equal(
    nrow(sleep.lme.caseD$vcov.delete),
    nrow(as.matrix(vcov(sleep.lme)))
  )
  expect_equal(
    ncol(sleep.lme.caseD$vcov.delete),
    ncol(as.matrix(vcov(sleep.lme)))
  )
  ...
  expect_equal(
    nrow(chem.lme.case$vcov.delete[[1]]),
    nrow(as.matrix(vcov(chem.lme)))
  )
  expect_equal(
    ncol(chem.lme.case$vcov.delete[[1]]),
    ncol(as.matrix(vcov(chem.lme)))
  )
})
...
test_that("Dimensions of variance covariance matrices after deletion are correct for group deletion when delete param is NULL for lme4 models", {
  #sleepstudy
  expect_equal(
    length(sleep.lmer.caseG$vcov.delete),
    length(unique(sleep.lmer@flist[["Subject"]]))
  )
  expect_equal(
    nrow(sleep.lmer.caseG$vcov.delete[[1]]),
    nrow(as.matrix(vcov(sleep.lmer)))
  )
  expect_equal(
    ncol(sleep.lmer.caseG$vcov.delete[[1]]),
    ncol(as.matrix(vcov(sleep.lmer)))
  )

  #chemistry
  expect_equal(
    length(chem.lmer.caseG$vcov.delete),
    length(unique(chem.lmer@flist[["lea"]]))
  )
  expect_equal(
    nrow(chem.lmer.caseG$vcov.delete[[1]]),
    nrow(as.matrix(vcov(chem.lmer)))
  )
  expect_equal(
    ncol(chem.lmer.caseG$vcov.delete[[1]]),
    ncol(as.matrix(vcov(chem.lmer)))
  )

  expect_equal(
    length(chem.lmer.caseG2$vcov.delete),
    length(unique(chem.lmer@flist[["school:lea"]]))
  )
  expect_equal(
    nrow(chem.lmer.caseG2$vcov.delete[[1]]),
    nrow(as.matrix(vcov(chem.lmer)))
  )
  expect_equal(
    ncol(chem.lmer.caseG2$vcov.delete[[1]]),
    ncol(as.matrix(vcov(chem.lmer)))
  )
})
...
test_that("Dimensions of variance covariance matrices after deletion are correct for group deletion when delete param is NULL for nlme models", {
  #sleepstudy
  expect_equal(
    length(sleep.lme.caseG$vcov.delete),
    length(unique(sleep.lme$groups$Subject))
  )
  expect_equal(
    nrow(sleep.lme.caseG$vcov.delete[[1]]),
    nrow(as.matrix(vcov(sleep.lme)))
  )
  expect_equal(
    ncol(sleep.lme.caseG$vcov.delete[[1]]),
    ncol(as.matrix(vcov(sleep.lme)))
  )

  #chemistry
  expect_equal(
    length(chem.lme.caseG$vcov.delete),
    length(unique(chem.lme$groups[["lea"]]))
  )
  expect_equal(
    nrow(chem.lme.caseG$vcov.delete[[1]]),
    nrow(as.matrix(vcov(chem.lme)))
  )
  expect_equal(
    ncol(chem.lme.caseG$vcov.delete[[1]]),
    ncol(as.matrix(vcov(chem.lme)))
  )

  expect_equal(
    length(chem.lme.caseG2$vcov.delete),
    length(unique(chem.lme$groups[["school"]]))
  )
  expect_equal(
    nrow(chem.lme.caseG2$vcov.delete[[1]]),
    nrow(as.matrix(vcov(chem.lme)))
  )
  expect_equal(
    ncol(chem.lme.caseG2$vcov.delete[[1]]),
    ncol(as.matrix(vcov(chem.lme)))
  )
})
...
test_that("Dimensions of variance covariance matrices after deletion are correct when delete param is set for lme4 models", {
  #sleepstudy
  expect_equal(
    nrow(sleep.lmer.caseGD$vcov.delete),
    nrow(as.matrix(vcov(sleep.lmer)))
  )
  expect_equal(
    ncol(sleep.lmer.caseGD$vcov.delete),
    ncol(as.matrix(vcov(sleep.lmer)))
  )

  #chemistry
  expect_equal(
    nrow(chem.lmer.caseGD$vcov.delete),
    nrow(as.matrix(vcov(chem.lmer)))
  )
  expect_equal(
    ncol(chem.lmer.caseGD$vcov.delete),
    ncol(as.matrix(vcov(chem.lmer)))
  )

  expect_equal(
    nrow(chem.lmer.caseGD2$vcov.delete),
    nrow(as.matrix(vcov(chem.lmer)))
  )
  expect_equal(
    ncol(chem.lmer.caseGD2$vcov.delete),
    ncol(as.matrix(vcov(chem.lmer)))
  )
})
...
test_that("Dimensions of variance covariance matrices after deletion are correct when delete param is set for nlme models", {
  #sleepstudy
  expect_equal(
    nrow(sleep.lme.caseGD$vcov.delete),
    nrow(as.matrix(vcov(sleep.lme)))
  )
  expect_equal(
    ncol(sleep.lme.caseGD$vcov.delete),
    ncol(as.matrix(vcov(sleep.lme)))
  )

  #chemistry
  expect_equal(
    nrow(chem.lme.caseGD$vcov.delete),
    nrow(as.matrix(vcov(chem.lme)))
  )
  expect_equal(
    ncol(chem.lme.caseGD$vcov.delete),
    ncol(as.matrix(vcov(chem.lme)))
  )

  expect_equal(
    nrow(chem.lme.caseGD2$vcov.delete),
    nrow(as.matrix(vcov(chem.lme)))
  )
  expect_equal(
    ncol(chem.lme.caseGD2$vcov.delete),
    ncol(as.matrix(vcov(chem.lme)))
  )
})
...
test_that("Dimenstions of variance components are correct for single case deletion for lme4 models", {
  ...
  expect_equal(
    length(sleep.lmer.case$varcomp.delete[[1]]),
    length(varcomp.mer(sleep.lmer))
  )
  expect_equal(
    length(sleep.lmer.caseD$varcomp.delete),
    length(varcomp.mer(sleep.lmer))
  )
  ...
  expect_equal(
    length(chem.lmer.case$varcomp.delete[[1]]),
    length(varcomp.mer(chem.lmer))
  )
})
...
test_that("Dimenstions of variance components are correct for single case deletion for nlme models", {
  ...
  expect_equal(
    length(sleep.lme.case$varcomp.delete[[1]]),
    length(varcomp.lme(sleep.lme))
  )
  expect_equal(
    length(sleep.lme.caseD$varcomp.delete),
    length(varcomp.lme(sleep.lme))
  )
  ...
  expect_equal(
    length(chem.lme.case$varcomp.delete[[1]]),
    length(varcomp.lme(chem.lme))
  )
})
...
test_that("Dimensions of variance components are correct for group deletion when delete param is NULL for lme4 models", {
  #sleepstudy
  expect_equal(
    length(sleep.lmer.caseG$varcomp.delete),
    length(unique(sleep.lmer@flist[["Subject"]]))
  )
  expect_equal(
    length(sleep.lmer.caseG$varcomp.delete[[1]]),
    length(varcomp.mer(sleep.lmer))
  )

  #chemistry
  expect_equal(
    length(chem.lmer.caseG$varcomp.delete),
    length(unique(chem.lmer@flist[["lea"]]))
  )
  expect_equal(
    length(chem.lmer.caseG$varcomp.delete[[1]]),
    length(varcomp.mer(chem.lmer))
  )

  expect_equal(
    length(chem.lmer.caseG2$varcomp.delete),
    length(unique(chem.lmer@flist[["school:lea"]]))
  )
  expect_equal(
    length(chem.lmer.caseG2$varcomp.delete[[1]]),
    length(varcomp.mer(chem.lmer))
  )
})
...
test_that("Dimensions of variance components are correct for group deletion when delete param is NULL for nlme models", {
  #sleepstudy
  expect_equal(
    length(sleep.lme.caseG$varcomp.delete),
    length(unique(sleep.lme$groups$Subject))
  )
  expect_equal(
    length(sleep.lme.caseG$varcomp.delete[[1]]),
    length(varcomp.lme(sleep.lme))
  )

  #chemistry
  expect_equal(
    length(chem.lme.caseG$varcomp.delete),
    length(unique(chem.lme$groups[["lea"]]))
  )
  expect_equal(
    length(chem.lme.caseG$varcomp.delete[[1]]),
    length(varcomp.lme(chem.lme))
  )

  expect_equal(
    length(chem.lme.caseG2$varcomp.delete),
    length(unique(chem.lme$groups[["school"]]))
  )
  expect_equal(
    length(chem.lme.caseG2$varcomp.delete[[1]]),
    length(varcomp.lme(chem.lme))
  )
})
...
test_that("Dimensions of variance components are correct for group deletion when delete param is set for lme4 models", {
  #sleepstudy
  expect_equal(
    length(sleep.lmer.caseGD$varcomp.delete),
    length(varcomp.mer(sleep.lmer))
  )

  #chemistry
  expect_equal(
    length(chem.lmer.caseGD$varcomp.delete),
    length(varcomp.mer(chem.lmer))
  )
  expect_equal(
    length(chem.lmer.caseGD2$varcomp.delete),
    length(varcomp.mer(chem.lmer))
  )
})
...
test_that("Dimensions of variance components are correct for group deletion when delete param is set for nlme models", {
  #sleepstudy
  expect_equal(
    length(sleep.lme.caseGD$varcomp.delete),
    length(varcomp.lme(sleep.lme))
  )

  #chemistry
  expect_equal(
    length(chem.lme.caseGD$varcomp.delete),
    length(varcomp.lme(chem.lme))
  )
  expect_equal(
    length(chem.lme.caseGD2$varcomp.delete),
    length(varcomp.lme(chem.lme))
  )
})
...
test_that("Only correct arguments for delete parameter are allowed for lme4 models", {
  ...
  expect_error(
    case_delete(sleep.lmer, delete = "308"),
    "parameter should be a numeric vector"
  )
  expect_error(
    case_delete(chem.lmer, delete = "1"),
    "parameter should be a numeric vector"
  )

  #for group, numeric indices or group names (as a character vector) are allowed
  expect_warning(
    case_delete(sleep.lmer, level = "Subject", delete = c(1, 2, 11)),
    "deleted cases do not encompass entire groups"
  )
  expect_error(
    case_delete(chem.lmer, level = "school:lea", delete = c("1/1", "2/5")),
...
test_that("Only correct arguments for delete parameter are allowed for nlme models", {
  #for single case, only numeric cases are allowed
  expect_error(
    case_delete(sleep.lme, delete = "308"),
    "parameter should be a numeric vector"
  )
  expect_error(
    case_delete(chem.lme, delete = "1"),
    "parameter should be a numeric vector"
  )

  #for group, numeric indices or group names (as a character vector) are allowed
  expect_warning(
    case_delete(sleep.lme, level = "Subject", delete = c(1, 2, 11)),
    "deleted cases do not encompass entire groups"
  )
  expect_error(
    case_delete(chem.lme, level = "school", delete = c("1:1", "5:2")),
    "not a valid group name"
  )
})
