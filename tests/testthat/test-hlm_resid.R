...
bdf <- nlme::bdf
bdf.lmer <- lme4::lmer(
  IQ.verb ~ ses + aritPOST + langPOST + schoolSES + (1 | schoolNR),
  data = bdf
)
bdf.lme <- nlme::lme(
  IQ.verb ~ ses + aritPOST + langPOST + schoolSES,
  random = ~ 1 | schoolNR,
  data = bdf
)
...
test_that("standardize works, lme4", {
  ...
  #check column names
  expect_equal(
    names(bdf.resids.lmer.lvl1[(nc - 5):nc]),
    c(
      ".resid",
      ".fitted",
      ".ls.resid",
  ...
  expect_equal(
    names(bdf.resids.lmer.lvl1.std[(nc - 5):nc]),
    c(
      ".std.resid",
      ".fitted",
      ".std.ls.resid",
  ...
  expect_false(all(
    bdf.resids.lmer.lvl1$.resid == bdf.resids.lmer.lvl1.std$.std.resid
  ))
  expect_false(all(
    bdf.resids.lmer.lvl1$.ls.resid == bdf.resids.lmer.lvl1.std$.std.ls.resid
  ))
  expect_false(all(
    bdf.resids.lmer.lvl1$.mar.resid == bdf.resids.lmer.lvl1.std$.chol.mar.resid
  ))
  expect_true(all(
    bdf.resids.lmer.lvl1$.fitted == bdf.resids.lmer.lvl1.std$.fitted
  ))
})
...
test_that("standardize works, nlme", {
  ...
  #check column names
  expect_equal(
    names(bdf.resids.lme.lvl1[(nc - 5):nc]),
    c(
      ".resid",
      ".fitted",
      ".ls.resid",
  ...
  expect_equal(
    names(bdf.resids.lme.lvl1.std[(nc - 5):nc]),
    c(
      ".std.resid",
      ".fitted",
      ".std.ls.resid",
  ...
  #check that raw is not the same as std
  expect_false(all(
    bdf.resids.lme.lvl1$.resid == bdf.resids.lme.lvl1.std$.std.resid
  ))
  expect_false(all(
    bdf.resids.lme.lvl1$.ls.resid == bdf.resids.lme.lvl1.std$.std.ls.resid
  ))
  expect_false(all(
    bdf.resids.lme.lvl1$.mar.resid == bdf.resids.lme.lvl1.std$.chol.mar.resid
  ))
  expect_true(all(
    bdf.resids.lme.lvl1$.fitted == bdf.resids.lme.lvl1.std$.fitted
  ))
})
...
test_that("3 level model tests, lme4", {
  ...
  class.lmer <- lme4::lmer(
    mathgain ~ mathkind +
      minority +
      ses +
      housepov +
      (mathkind | schoolid / classid),
    classroom
  )
  ...
  #grabs only two level variables
  expect_equal(
    names(class.resids)[1:4],
    c("group", "classid", "schoolid", "housepov")
  )
...
test_that("3 level model tests, nlme", {
  ...
  class.lme <- nlme::lme(
    mathgain ~ mathkind + minority + ses + housepov,
    random = ~ mathkind | schoolid / classid,
    classroom
  )
  ...
  expect_equal(
    names(class.resids)[1:4],
    c("group", "schoolid", "classid", "housepov")
  )
...
class.lmeNA <- nlme::lme(
  mathgain ~ mathkind + minority + mathknow + ses + housepov,
...
test_that("respects na.action, lme4", {
  expect_equal(
    nrow(expect_warning(hlm_resid(class.lmerNA, data = classroom))),
    nrow(classroom)
  )
  expect_equal(
    nrow(hlm_resid(class.lmerNA, data = classroom, include.ls = FALSE)),
    nrow(classroom)
  )
})
...
test_that("respects na.action, nlme", {
  expect_equal(
    nrow(expect_warning(hlm_resid(class.lmeNA, data = classroom))),
    nrow(classroom)
  )
  expect_equal(
    nrow(hlm_resid(class.lmeNA, data = classroom, include.ls = FALSE)),
    nrow(classroom)
  )
})
...
Exam <- mlmRev::Exam
exam.lmer <- lme4::lmer(
  normexam ~ standLRT + schgend + (1 | school),
  data = Exam
)
exam.lme <- nlme::lme(
  normexam ~ standLRT + schgend,
  random = ~ 1 | school,
  data = Exam
)
...
test_that("correct dimentions- simple model, lme4", {
  ...
  expect_equal(nrow(hlm_resid(exam.lmer, standardize = TRUE)), nrow(Exam))
  expect_equal(
    nrow(hlm_resid(exam.lmer, standardize = TRUE, include.ls = FALSE)),
    nrow(Exam)
  )
  ...
  #check lvl2
  expect_equal(
    nrow(hlm_resid(exam.lmer, level = "school", include.ls = FALSE)),
...
test_that("correct dimentions- simple model, nlme", {
  ...
  expect_equal(nrow(hlm_resid(exam.lme, standardize = TRUE)), nrow(Exam)) #this one takes mega long for some reason
  expect_equal(
    nrow(hlm_resid(exam.lme, standardize = TRUE, include.ls = FALSE)),
    nrow(Exam)
  ) #this one also takes mega long - surprising
  expect_equal(nrow(hlm_resid(exam.lme, include.ls = FALSE)), nrow(Exam))

  #check lvl2
  expect_equal(
    nrow(hlm_resid(exam.lme, level = "school", include.ls = FALSE)),
    length(unique(Exam$school))
  )
  expect_equal(
    nrow(hlm_resid(
