...
test_that("Number of rows and columns are correct for full refits for lme4 models", {
  skip_on_cran()
  #sleepstudy
  sleep.lmer.infl2 <- suppressWarnings(hlm_influence(
    sleep.lmer,
    approx = FALSE
  ))
  expect_equal(
    ncol(sleep.lmer.infl2),
    6 + ncol(sleep.lmer@frame) + length(HLMdiag:::varcomp.mer(sleep.lmer))
  )

  #chemistry
  chem.lmer.infl2 <- suppressWarnings(suppressMessages(hlm_influence(
    chem.lmer,
    approx = FALSE
  )))
  expect_equal(
    ncol(chem.lmer.infl2),
    6 + ncol(chem.lmer@frame) + length(HLMdiag:::varcomp.mer(chem.lmer))
  )
})
...
test_that("Number of rows and columns are correct for full refits for nlme models", {
  ...
  expect_equal(
    ncol(sleep.lme.infl2),
    6 + ncol(sleep.lmer@frame) + length(HLMdiag:::varcomp.lme(sleep.lme))
  )
  ...
  expect_equal(
    ncol(chem.lme.infl2),
    6 + ncol(chem.lmer@frame) + length(HLMdiag:::varcomp.lme(chem.lme))
  )
})

test_that("Number of columns is correct when leverage is specified for lme4 models", {
  #sleepstudy
  sleep.lmer.infl <- hlm_influence(
    sleep.lmer,
...
test_that("Number of rows and columns are correct when delete is specified for lme4 models", {
  ...
  expect_warning(hlm_influence(
    sleep.lmer,
    delete = c(1, 10, 13),
    leverage = c("ranef", "ranef.uc")
  ))
  expect_warning(hlm_influence(
    sleep.lmer,
    delete = c(1, 10, 13),
