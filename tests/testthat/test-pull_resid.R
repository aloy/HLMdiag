...
test_that("ls results match hlm_resid, lme4", {
  expect_equal(
    expect_warning(pull_resid(bdf.lmer)),
    bdf.resids.lmer.raw$.ls.resid
  )
  expect_equal(
    expect_warning(pull_resid(bdf.lmer, standardize = TRUE)),
...
test_that("ls results match hlm_resid, nlme", {
  expect_equal(
    expect_warning(pull_resid(bdf.lme)),
    bdf.resids.lme.raw$.ls.resid
  )
  expect_equal(
    expect_warning(pull_resid(bdf.lme, standardize = TRUE)),
    bdf.resids.lme.std$.std.ls.resid
  )
})

test_that("eb results match hlm_resid, lme4", {
  expect_equal(pull_resid(bdf.lmer, type = "eb"), bdf.resids.lmer.raw$.resid)
  expect_equal(
    pull_resid(bdf.lmer, type = "eb", standardize = TRUE),
    bdf.resids.lmer.std$.std.resid
  )
})

test_that("eb results match hlm_resid, nlme", {
  expect_equal(pull_resid(bdf.lme, type = "eb"), bdf.resids.lme.raw$.resid)
  expect_equal(
    pull_resid(bdf.lme, type = "eb", standardize = TRUE),
...
test_that("marginal results match hlm_resid, lme4", {
  expect_equal(
    pull_resid(bdf.lmer, type = "marginal"),
    bdf.resids.lmer.raw$.mar.resid
  )
  expect_equal(
    pull_resid(bdf.lmer, type = "marginal", standardize = TRUE),
...
test_that("marginal results match hlm_resid, lme4", {
  expect_equal(
    pull_resid(bdf.lme, type = "marginal"),
    bdf.resids.lme.raw$.mar.resid
  )
  expect_equal(
    pull_resid(bdf.lme, type = "marginal", standardize = TRUE),
